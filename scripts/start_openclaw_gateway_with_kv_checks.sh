...
REQUEST_TIMEOUT_DEFAULT="${OPENCLAW_OLLAMA_REQUEST_TIMEOUT_MS:-90000}"
MAX_RETRIES_DEFAULT="${OPENCLAW_OLLAMA_MAX_RETRIES:-1}"
...
if not isinstance(paths, list):
    paths = []
filtered_paths = [p for p in paths if p != legacy_shadow_plugin_path]
...
if shadow_plugin_path not in paths:
    ...
    changed = True
if paths and paths[0] != shadow_plugin_path:
    paths = [shadow_plugin_path, *[p for p in paths if p != shadow_plugin_path]]
    changed = True
load["paths"] = paths
