[2026-04-14] - Root Cause Fix: Ingest Scripts Moved from LLM-Mediated Agent Turns to Direct launchd Agents
Problem
polly_ingest.py (every 20m) and maxwell_ingest.py (every 30m) were invoked by asking a qwen2.5:7b LLM agent to "execute this command via exec." This created multiple unnecessary failure modes: the model had to generate correct exec parameters on every run (leading to the security:allowlist and elevated failures); each invocation consumed a gateway event loop slot for 10-15s, causing all other pending agent callbacks to wait; a Telegram polling stall or slow Ollama inference during one of these cycles cascaded into 459s-955s backer hangs; and session lock files accumulated from isolated cron sessions. The ingest scripts contain zero LLM reasoning — they are pure Python data pipeline work (file I/O + SQLite writes) that never required an agent turn in the first place.
Root Cause
Architectural: deterministic pipeline work was routed through the LLM agent gateway, which exists for reasoning tasks. The gateway event loop is single-threaded; blocking it with script execution starved all other agents.
Solution
Created launchd/com.openclaw.polly-ingest.plist and launchd/com.openclaw.maxwell-ingest.plist. These are native macOS background agents (StartInterval=1200s and 1800s respectively) that invoke python3 polly_ingest.py --verbose and python3 maxwell_ingest.py --verbose directly, with zero LLM involvement. Disabled ingestion-watch-20m and maxwell-ingest-30m in ~/.openclaw/cron/jobs.json (_disabledReason field added). Created scripts/install_ingest_launchd.sh: install/uninstall/status script for managing the two agents. Updated OPENCLAW_HOW_IT_SHOULD_WORK.md §7 to reflect the disabled cron jobs and added §22 documenting the launchd agent pattern and the rule against LLM-mediated data pipeline work. Install with: bash ~/openclaw/scripts/install_ingest_launchd.sh
Notes
The gateway still runs polly-pre-digest-healthcheck (6:50am) and polly-digest-prep (6:55am) as agent turns — these are correct uses because they involve model judgment (health assessment, digest assembly). Only the mechanical 20m/30m ingest cycles were moved. Log rotation is not configured in the plist — the backer nightly audit can be extended to truncate these logs if they grow large. The --verbose flag is intentional so logs contain per-run detail for debugging.

[2026-04-14] - Script-Level Lock Files to Prevent Overlapping Cron Invocations
Problem
polly_ingest.py (runs every 20 minutes) and maxwell_ingest.py (runs every 30 minutes) could overlap at their LCM (every 60 minutes) and whenever a prior run was slow. Both scripts write to polly.db — SQLite WAL allows concurrent readers but serializes writers. An overlapping second invocation would queue behind the first, causing the cron job to run for much longer than expected, triggering Telegram/gateway event loop congestion. In extreme cases (Telegram polling stall or gateway restart), overlapping script timeouts cascaded into 459s–955s backer hangs.
Root Cause
No inter-process coordination between cron invocations of the same script. A new invocation started regardless of whether a prior one was still running. The only existing lock mechanism was detect_stale_session_locks() in backer_health_tick.sh, which operates on gateway session .jsonl.lock files — not on external script processes.
Solution
Created scripts/script_lock.py: a reusable advisory lock utility using fcntl.flock() with LOCK_EX|LOCK_NB (non-blocking exclusive). Lock files land in /tmp/openclaw/<name>.lock and contain JSON with the holder's PID and start timestamp. If the lock is already held by a live process, AlreadyRunning is raised immediately (no hang). If the PID in the file is dead (stale lock from a crash), it is automatically cleared and the lock is re-acquired. Added script_lock("polly-ingest") to polly_ingest.py main() guard. Added script_lock("maxwell-ingest") to maxwell_ingest.py main() guard. Both print {"status": "skipped", "reason": "..."} and exit 0 on AlreadyRunning — cron treat this as a successful no-op. Added clear_stale_script_locks() to backer_health_tick.sh: every 5-minute health check uses flock(LOCK_NB) to detect lock files whose holder process is dead and removes them. The cleared count is included in the health summary JSON (stale_script_locks_cleared field).
Notes
fcntl.flock() is advisory and process-scoped — the OS automatically releases it if the process dies (even on SIGKILL), so stale locks from crashes are self-healing within at most one health-tick cycle (5 minutes). The clear_stale_script_locks() function in backer_health_tick.sh adds an explicit belt-and-suspenders cleanup for any edge cases. Lock files are removed on clean exit via ScriptLock.release(). The /tmp/openclaw/ directory is created with parents=True, exist_ok=True on first use.
...
[2026-04-13] - Gateway Restart Cascade: Stale Locks + Stuck Process + Misleading Error
Problem
Polly returned "No API provider registered for api: ollama" on Telegram. Attempting to restart the gateway failed repeatedly with EADDRINUSE because the old process (pid 63996) ignored SIGTERM and launchd kept the port held.
Root Cause
Three independent failures cascaded: (1) A stale .jsonl.lock file from a dead process (pid 10196) blocked Polly's session on the ollama/gemma4:26b lane — the lock owner was dead but the file persisted. (2) The fallback to openai-codex/gpt-5.3-codex failed because Codex credits were exhausted (rate limited). (3) The gateway process itself became stuck (likely on the same lock contention) and ignored SIGTERM, so `launchctl stop` appeared to work but left the process alive. The "No API provider registered for api: ollama" error was a misleading wrapper around the real FallbackSummaryError. The startup script (start_openclaw_gateway_with_kv_checks.sh) had no pre-flight checks: it did not verify the old gateway was actually stopped, did not clean stale session locks, and did not check Ollama lane health before launching.
Solution
